...
-- Add SMS frequency and last_sms_sent_at columns if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'sms_frequency'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN sms_frequency TEXT DEFAULT 'daily';
    -- daily, weekly, none
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'last_sms_sent_at'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN last_sms_sent_at TIMESTAMPTZ;
  END IF;
END $$;
